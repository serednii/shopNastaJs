...
function generateCards(data, clases) {
    ...
    data.forEach(el => {
        ...
        <a href="#" class="card__name">${el.title.replace(/Характеристики/g, "")}</a>
        <div class="card__img-wrapper">
            <a href="${PRODUCTHTML}" class="card__img"><img class="product-img" src="${el.images[0].img} " alt="${el.images[0].alt}"></a>
        </div >
        <div class="card__price">
            <s class="card__price-old"><span class="valuta">UA</span> <span
                    class="cena">${el.oldPrice}</span></s>
            <span class="card__price-sale"><span class="valuta">UA</span> <span class="cena">${el.newPrice}</span></span>
        </div>
